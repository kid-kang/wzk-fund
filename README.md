...
| 模块 | 说明 |
|------|------|
| 持仓列表 | 仅基金：境内 / QDII 分区汇总；单基金额、收益、涨跌幅、收益率、板块标签、迷你走势；点名称进基金详情；左滑「操作」进加减仓定投，「删除」清账本并转入自选 |
| 黄金 | 独立于基金结算；实时市值、克数/均价/成本、分时金价；只维护持仓口径，不记交易流水 |
| 自选基金 | 名称代码、涨跌幅、板块标签、迷你走势；点名称进基金详情；左滑删除自选（交易点位保留） |
| 指数看板 | 上证/深证/创业板/北证50/科创50/上证50/沪深300/中证500/纳斯达克100/标普500 |
| 行情 · 板块 | 涨跌家数；主力资金净流入；今日板块热搜 / 涨幅榜；基金热榜；点板块进「板块热搜基金」二级页 |

添加持仓需「代码或名称 + 当前持仓金额 + 投入成本」（买入日期选填）；添加自选只需「代码或名称」。名称可检索候选后点选，板块与净值自动拉取。「我的」可查看收益/交易记录，并导入/导出完整本地配置。

### 近期能力

| 能力 | 说明 |
|------|------|
| 持仓操作账本 | 持仓左滑「操作」：持仓信息 / 加仓 / 减仓 / 定投；净值未公布时流水为待确认，公布后落账；减仓可快捷选 1/4、1/3、1/2、2/3、3/4、清仓 |
| 删除持仓 vs 自选 | 持仓「删除」会清掉该基金交易/定投/买卖点并转入自选，**收益记录按日独立保留**；自选「删除」只移出自选，交易点位仍在 |
| 名称检索 | 添加持仓/自选可输入基金名称或代码片段，点选候选后写入 6 位代码 |
| 交易 / 收益记录 | 「我的」进入：交易记录看加减仓与定投流水；收益记录看功能上线后按天记下的组合盈亏 |
...
| 净值曲线 | 近1月 / 近3月 / 近6月 / 近1年 / 近3年 / 成立来（按成立天数隐藏不可用周期）；周期胶囊展示区间涨跌；高低点同色标注涨跌幅（带白圈）；有交易时标买入（红）/ 卖出（蓝）/ 清仓（绿）色点，不在点上叠涨跌数字（避免定投过密）；图例始终显示；「来财」水印 |
...
录入或修改持仓时填写**当前持仓金额**（支付宝确认后的市值）和**投入成本**（用于收益率）。系统按**最新已公布单位净值**反推份额，本地保存 `shares` 与 `cost`。拉不到净值则保存失败，提示重试。
...
- 金额、份额、费率输入会过滤非数字（开发者工具与粘贴也不保留字母）。

### 基金 · 加仓 / 减仓 / 定投

在持仓页左滑「操作」记账；添加持仓时填写的金额 / 成本仍是账本事实，加仓减仓只改份额与流水。

| 操作 | 规则 |
|------|------|
| 加仓 | 填写买入金额与费率；15:00 前按申请日净值，15:00 后按下一日；净值未公布则流水为待确认，公布后落账 |
| 减仓 | 填写卖出份额（可快捷 1/4～3/4 或清仓）；卖出日期不得早于买入日期；净值规则同加仓 |
| 定投 | 选定投金额、费率与周期；系统计算下次扣款日；改周期会生成新计划，不沿用旧日程；可暂停 / 终止 |
| QDII | 同一套 15:00 切点，等待申请日净值（通常 T+1 披露），不用更早净值顶替 |
| 黄金 | 只维护克数与均价，不写交易记录 |

加减仓确认前有二次弹窗（不可撤销）。已产生的**按日收益记录**不随单只基金删除。
...
|----|------|
| 存储键 | `wzk-fund-config` |
| 内容 | `settings`（黄金显示、`profitSince`）、`funds`（持仓/自选）、`gold`、`profitLog`（按日收益）、`trades`（加减仓/定投流水）、`sipPlans` |
...
1. **添加/编辑持仓**  
   - 代码可手输 6 位，或按名称检索后点选。  
   - 调 `resolve` 取最新确认净值。  
   - 用当前持仓金额反推 `shares`，连同 `cost`、可选 `buyDate`、`code`、`name` 写入本地。  

2. **删除持仓**  
   - 侧滑「删除」：清掉该基金的交易、定投、买卖点，份额/成本归零并转入自选。  
   - `profitLog` 不删（按日独立记账）。  

3. **删除自选**  
   - 只从自选列表移除；交易记录和走势图买卖点保留。  

4. **黄金仓位**  
   - 只更新 `gold.holding`、`gold.avgPrice` 与费率；不写 `trades`。  

5. **黄金显示开关 / 主题等**  
   - 写 `settings`（如 `showGold`）。  

6. **行情刷新**  
   - 基金持仓金额和收益只由 `shares` 与最新行情实时计算；刷新不会改写 `shares`。刷新时会尝试给待确认交易落账、补记到期定投。  
...
- `GET /api/funds/suggest?q=`（代码片段或名称联想，添加持仓/自选用）
- `GET /api/market/trading-days`（沪深交易日，定投/买卖日期校验）
...
- `GET /api/market/money-flow`（主力资金净流入）
- `GET /api/market/funds`（行情基金榜）
...
| Tab | 说明 |
|-----|------|
| 持仓 | 境内 / QDII 分区汇总与列表（金额、占比、盈亏、涨跌、收益率、可点板块、迷你走势）；点名称或迷你图进基金详情；左滑「操作」（加减仓定投）/「删除」（清账本转入自选，收益记录保留）；可选黄金模块；约 30s 静默轮询 |
| 自选 | 自选列表（涨跌、可点板块标签、迷你走势）；点名称或迷你图进基金详情；右下角 FAB 添加；左滑删除（交易点位保留）；导航标题 `自选(N)`；同样支持轮询 |
| 行情 | A 股涨跌家数；主力资金净流入；今日板块热搜 / 涨幅榜（点板块进热搜基金页）；基金榜；指数网格 |
| 我的 | 黄金显示、隐藏金额、深色主题；收益记录 / 交易记录；Q&A；API 地址保存与连通探测；剪贴板导入/导出；清空持仓/自选/全部；`wzk-fund` 水印 |

### 二级页

| 页面 | 入口 | 说明 |
|------|------|------|
| 基金表单 | 持仓 `+`；自选 FAB | `hold`：代码或名称 + 当前持仓金额 + 投入成本 + 可选买入日期；`watch`：代码或名称。满 6 位或点选检索结果后 `resolve` |
| 持仓操作 | 持仓左滑「操作」 | 持仓信息（金额/成本/收益率只读，买入日期可改）/ 加仓 / 减仓 / 定投；待确认落账；管理定投 |
| 交易记录 | 我的 · 交易记录 | 加仓、减仓、清仓、定投流水；待确认状态 |
| 收益记录 | 我的 · 收益记录 | 功能上线后按天记下的组合盈亏；与单只基金删除无关 |
| 基金详情 | 持仓/自选名称或迷你图；板块基金列表 | 报头（含可点板块标签）+ 净值曲线（图例、高低点、买卖清仓色点）；**重仓股票前十**；数据看板（历史净值、阶段涨幅、阶段回撤、规模） |
| 板块热搜基金 | 行情板块行；持仓/自选/详情的板块标签 | 该板块相关热搜基金列表；加自选；点行进基金详情；跟随深浅色主题 |
| 黄金设置 | 持仓黄金区设置 | 买入/卖出费率、持有克数、成本均价；不记交易流水 |
| 金价走势 | 持仓实时金价 | 分时 / 近1月 / 近3月 / 近6月 / 近1年；价格轴 + 高低点 |
| 指数走势 | 行情指数卡片 | 近1月 / 近3月 / 近6月 / 近1年 / 近3年；样式对齐基金详情 |
| Q&A | 我的；持仓/自选徽章；添加持仓说明 | 口径与常见问题（确认净值、QDII、加仓规则等） |

### 本地存储与真机

| 键 | 说明 |
|----|------|
| `wzk-fund-config` | funds / gold / settings / profitLog / trades / sipPlans |
