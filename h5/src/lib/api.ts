...
import {getAxiosBaseURL} from '@/lib/config'
...
export function syncApiBase() {
  api.defaults.baseURL = getAxiosBaseURL()
}
...
export type GoldHistoryRange = '1m' | '3m' | '6m' | '1y'

export type GoldHistoryPayload = {
  code?: string
  name?: string
  range: GoldHistoryRange | string
  periodPercent: number | null
  points: {date: string; close?: number; price?: number; percent?: number | null}[]
}

export async function fetchGoldHistory(range: GoldHistoryRange | string = '1m') {
  const {data} = await api.get<{
    success: boolean
    message?: string
    data: GoldHistoryPayload
  }>('/gold/history', {params: {range}})
  assertOk(data)
  return data.data
}

export async function healthCheck() {
  const {data} = await api.get<{success?: boolean; ok?: boolean; message?: string}>(
    '/health',
  )
  return data
}
